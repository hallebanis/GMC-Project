...
router.delete('/deleteReservation/:id', (req, res) => {
    newReservation.findByIdAndDelete(req.params.id)
        .then(() => res.json({ msg: 'Reservation Deleted' }))
        .catch(err => console.log(err.message))
})
//Route  Update  reservation
router.put('/updateReservation/:id', (req, res) => {
    newReservation.findByIdAndUpdate(req.params.id, { $set: { ...req.body } }, (err, data) => {
        if (err) { throw err }
        newReservation.findById(req.params.id)
            .then(reservation => res.json(reservation))
            .catch(err => console.log(err.message))
    })
})
