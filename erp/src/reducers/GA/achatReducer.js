...
const achatReducer = (state = initState, action) => {
  switch (action.type) {
    ...
    case GET_PRODUIT_SUCCESS:
      return {
        ...state, errors: null, produit: action.payload
      }

    case DELETE_PRODUIT_SUCCESS:
      return {
        ...state, produit: state.produit.filter(el =>
          el._id !== action.payload._id
        )
      }
    case GET_FACTURE_SUCCESS:
      return {
        ...state, errors: null, facture: action.payload
      }
    case ADD_FACTURE_SUCCESS:
      return {
        ...state, facture: [...state.facture, action.payload], errors: null
      }
    case DELETE_FACTURE_SUCCESS:
      return {
        ...state, facture: state.facture.filter(el =>
          el._id !== action.payload._id
        )
      }
    case UPDATE_FACTURE_SUCCESS:
      return {
        ...state, errors: null, facture: state.facture.map(el => {
