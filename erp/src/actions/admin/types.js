export const LOAD_USERS_SUCCESS = "LOAD_USERS_SUCCESS";
export const LOAD_USERS_FAILED = "LOAD_USERS_FAILED";

export const MODIFY_USER_SUCCESS = "MODIFY_USER_SUCCESS";
export const MODIFY_USER_FAILED = "MODIFY_USER_FAILED";

export const DELETE_USER_SUCCESS = "DELETE_USER_SUCCESS";
export const DELETE_USER_FAILED = "DELETE_USER_FAILED";

export const ADD_USER_SUCCESS = "ADD_USER_SUCCESS";
export const ADD_USER_FAILED = "ADD_USER_FAILED";

export const LOAD_ROLES_SUCCESS = "LOAD_ROLES_SUCCESS";
export const LOAD_ROLES_FAILED = "LOAD_ROLES_FAILED";

export const DELETE_ROLE_SUCCESS = "DELETE_ROLE_SUCCESS";
export const DELETE_ROLE_FAILED = "DELETE_ROLE_FAILRD";

export const MODIFY_ROLE_FAILED = "MODIFY_ROLE_FAILED";
export const MODIFY_ROLE_SUCCESS = "MODIFY_ROLE_SUCCESS";

export const ADD_ROLE_SUCCESS = "ADD_ROLE_SUCCESS";
export const ADD_ROLE_FAILED = "ADD_ROLE_FAILED";

export const ADD_DEMANDE_SUCCESS = "ADD_DEMANDE_SUCCESS";
export const ADD_DEMANDE_FAILED = "ADD_DEMANDE_FAILED";

export const LOAD_PERSONNEL_ID_SUCCESS = "LOAD_PERSONNEL_ID_SUCCESS";
export const LOAD_PERSONNEL_ID_FAILED = "LOAD_PERSONNEL_ID_FAILED";
